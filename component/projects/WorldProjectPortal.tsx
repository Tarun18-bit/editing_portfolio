...
export default function WorldProjectPortal({ project, presentationStyle }: Props) {
  const { openProjectModal } = useWorld();
  ...
  const renderStyleWrapper = () => {
    switch (presentationStyle) {
      ...
      case "floating_widescreen":
        return "border border-blue-400/30 shadow-[0_0_80px_rgba(3,100,220,0.35)] rounded-2xl bg-slate-950/90 backdrop-blur-lg";
      case "dark_shadow":
  ...
  return (
    <div className="relative my-20 flex min-h-[85vh] w-full items-center justify-center px-4 md:px-8">
      <div
        ...
        className={`group relative cursor-pointer overflow-hidden transition-all duration-500 max-w-4xl w-full p-6 md:p-10 ${renderStyleWrapper()}`}
        style={{
